...
const VSCodePortfolio: React.FC = () => {
    ...
    const moveNext = (direction: "left" | "right" = "right") => {
        ...
        const xMove = direction === "right" ? 350 : -350;
        ...
        tl.to(outgoing, { x: xMove, opacity: 0, duration: 0.3, ease: "power2.out" })
            .to(outgoing, { y: 500, duration: 0.3, ease: "power2.in" });
    ...
    const movePrev = () => {
        ...
        if (incoming) {
            ...
            cardsElementsRef.current = cards;
            gsap.set(incoming, { x: 0, y: -500, scale: 1, opacity: 0, zIndex: 100 });
            ...
            tl.to(incoming, { y: 0, opacity: 1, duration: 0.6, ease: "back.out(1.2)" });
            cards.slice(1).forEach((card, i) => {
                tl.to(card, {
                    x: (i + 1) * gapX,
                    y: (i + 1) * gapY,
    ...
    const jumpToPage = (id: number) => {
        ...
        tl.to(outgoing, { x: 350, opacity: 0, stagger: 0.05, duration: 0.3, ease: "power2.out" })
    ...
    return (
        <section className="min-h-screen bg-[#FFEDD5] flex items-center justify-center p-4 md:p-10 select-none z-10 relative overflow-hidden font-sans text-white">
            ...
            {/* VS CODE WINDOW */}
            <div className="w-full max-w-6xl h-auto md:h-[750px] bg-[#121214] rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl flex flex-col md:flex-row overflow-hidden border border-white/5 relative">

                {/* TABLET TOGGLE (Moved inside the relative window) */}
                <button
                    onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                    className="absolute top-6 left-6 z-[110] hidden md:flex lg:hidden w-12 h-12 bg-[#18181b] rounded-full flex-col items-center justify-center gap-1 border border-white/10 shadow-lg active:scale-90 transition-transform"
                >
                ...
                {/* MAIN SLIDER AREA */}
                <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 bg-transparent relative">
                    <div
                        ref={sliderAreaRef}
                    ...
                    >
                        {cardData.map((card, i) => (
                            <div
                                ...
                                className={`absolute top-0 left-0 w-full h-64 md:h-80 card ${card.bg} ${card.text} shadow-2xl p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] will-change-transform flex flex-col`}
                            >
