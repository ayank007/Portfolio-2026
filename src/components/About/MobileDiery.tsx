...
const ScratchPadSlider = ({ data }: any) => {
    ...
    const handleNext = () => {
        ...
        animatePaper(next, 100);
    };

    const handlePrev = () => {
        const prev = (index - 1 + totalSlides) % totalSlides;
        animatePaper(prev, -100);
    };

    const animatePaper = (newIndex: number, xMove: number) => {
        gsap.to(paperRef.current, {
            x: xMove,
            opacity: 0,
            rotate: xMove > 0 ? 12 : -12,
            duration: 0.25,
            ease: "power2.in",
            onComplete: () => {
                setIndex(newIndex);
                gsap.fromTo(paperRef.current,
                    { x: -xMove, opacity: 0, rotate: xMove > 0 ? -12 : 12 },
                    { x: 0, opacity: 1, rotate: 2, duration: 0.4, ease: "back.out(1.4)" }
                );
            }
        });
    };
    ...
    const renderSlideContent = () => {
        switch (index) {
            case 0:
                return (
                    <>
                        <FormattedText text={data.diery.intro} />
                        <img src={me} alt="Me" className='mt-4 w-40 border-2 border-black shadow-md -rotate-2' />
                    </>
                );
            case 1:
                return (
                    <>
                        <p>
                            <FormattedText text={data.diery.reading} />
                        </p>
                        <div className="bookPreview">
                            <img src={shreekanta} alt="" />
                        </div>
                        <br />
                        <p>
                            <FormattedText text={data.diery.chess} />
                        </p>
            ...
            case 2:
                return (
                    <>
                        <p><FormattedText text={data.diery.music} /></p>
                        <p className='mt-4'><FormattedText text={data.diery.tv} /></p>
                        <div className="music">
            ...
            case 3:
                return (
                    <>
                        ...
                        <p><FormattedText text={data.diery.football} /></p>
                        <ul>
                            <li>Ronaldo - Kane - Messi</li>
                            <li>KDB - Busquets - Modric</li>
                            <li>Marcelo - Ramos - Terry - Lahm</li>
                            <li>Neuer</li>
                            <li className='text-xs'>Coach: Pep Guardiola</li>
                            <li className='text-xs'>Bench: Lewandowski, Neymar, Iniesta, Xavi, Kross, Kante, Thiago Silva, Dani Alves, Vidic, Buffon</li>
                        </ul>
                    </>
                );
            case 4:
                return (
                    <>
                        <p>
                            <FormattedText text={data.diery.sports} />
                        </p>
                        <p className='mt-4'><FormattedText text={data.diery.foodie} /></p>
                        <p className='mt-4'>
                            <FormattedText text={data.diery.travel} />
                        </p>
                        <div className="travel">
                            <div className="imgbox" style={{ backgroundImage: `url(${world})` }}>
            ...
            case 5:
                return (
                    <>
                        <p>
                            <FormattedText text={data.diery.starglazing} />
                        </p>
                        <br />
    ...
    return (
        <div className="flex flex-col items-center justify-center select-none w-full">
            <div className="relative w-full max-w-sm text-black">
                {/* Background "Stacked" sheets - height matches parent */}
                <div className="absolute inset-0 bg-white border-2 border-stone-900 rotate-1 translate-x-2 translate-y-2 rounded-sm shadow-sm" />
                <div className="absolute inset-0 bg-white border-2 border-stone-900 -rotate-2 -translate-x-1 translate-y-1 rounded-sm shadow-sm" />

                {/* Active Sheet - relative so height grows with content */}
                <div
                    ref={paperRef}
                    className="relative bg-stone-50 border-2 border-stone-900 p-8 pt-12 pb-6 text-sm rounded-sm shadow-xl flex flex-col rotate-2 w-full h-auto"
                >
                    {/* Notebook Spiral Holes */}
                    <div className="absolute top-4 left-0 right-0 flex justify-around px-4 opacity-40">
                        {[...Array(6)].map((_, i) => (
                            <div key={i} className="w-4 h-4 bg-stone-400 border-2 border-stone-900 rounded-full" />
                        ))}
                    </div>
                    ...
                    <p className="mt-8 text-center font-bold text-stone-400 border-t border-stone-200 pt-2 uppercase">
                        PAGE {index + 1}
                    </p>
                </div>
            </div>
            ...
            {/* Controls */}
            <div className="flex gap-8 mt-12 text-black">
                <button
                    onClick={handlePrev}
                    className="w-14 h-14 bg-white border-4 border-stone-900 flex items-center justify-center shadow-[4px_4px_0px_0px_rgba(28,25,23,1)] hover:bg-stone-900 hover:text-white transition-all active:translate-x-[2px] active:translate-y-[2px] active:shadow-none cursor-pointer"
                >
...
export default ScratchPadSlider;